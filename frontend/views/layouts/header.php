...
if (Yii::$app->user->isGuest) {
    ...
    $i = 0;
    $class = $i > 0 ? 'color-warning':'';
    $menuItems[] = [
                'label' => ($i > 0 ?'<span class="badge badge-danger">'.$i.'</span>&nbsp;':"").'<i class="far fa-bell '.$class.'"></i>',
                'items' => [
                    /*[
                        'label' => '<i class="glyphicon glyphicon-home"></i> Incidentes',
                        'url' => ['/admin/'],
                        #'active' => $action->controller->id == 'default',
                        'encode' => FALSE,
                    ]*/
                    [
                        'label' => 'No hay notificaciones pendientes',
                    ]
                ],
                'options' => [],
                'encode' => FALSE,
    ...
    $menuItems[] = [
        ...
        'items'=>[
            ['label'=>'<i class="fas fa-user"></i> Ver Perfil','url' => ['/user/profile'], 'encode'=> FALSE],
            ['label'=>'<i class="fas fa-sign-out-alt"></i> Cerrar Sesión','url' => ['/site/logout'], 'encode'=> FALSE],
        ],
        'encode'=> FALSE,
    ];
}
