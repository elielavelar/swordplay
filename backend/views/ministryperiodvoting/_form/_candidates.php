...
/* @var $modelDetail backend\models\Ministryperiodvotingcandidates */
/* @var $searchModel backend\models\MinistryperiodvotingcandidatesSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$url = Yii::$app->getUrlManager()->createUrl('ministryperiodvotingcandidate');
$urlMember = Yii::$app->getUrlManager()->createUrl('member');
        ...
        <div class='row'>
            <div class="col-md-12">
                <span class="pull-right">
                    <?= Html::button('<i class="fas fa-plus-circle"></i> Agregar', ['type' => 'button', 'class' => 'btn btn-success', 'id' => 'btnAddCandidate']) ?>
                </span>
            </div>
        </div>
        <?php endif; ?>
                       ...
                       [
                        ...
                        [
                            ...
                            'content' => function($model){
                                return $model->IdMember ? $model->member->displayName : '';
                            },
                        ],
                        [
                            'attribute' => 'IdState',
...
<?=$this->render('_modalCandidate', ['modelDetail' => $modelDetail, 'modalName' => $modalName, 'formName' => $formName, 'tableName' => $tableName])?>
<?php
$js = <<< JS
...
$jsHead = <<< JS
    ...
    var clearModal = function(){
        ...
        frm.ID = "$formName";
        var defaultvalues = {};
        $.extend(defaultvalues,{'$tableName-idvotation':$model->Id});
        frm.DEFAULTS = defaultvalues;
        frm.EXTRA = function(){
            $('#$tableName-idmember').val('').trigger('change');
        };
        clearForm(frm);
    };
